...
async function bootstrap() {
  ...
  if (
    ...
    !queryAll(".builder-question").length
  ) {
    addQuestionBlock();
  }
}
...
function bindDashboardScreens() {
  queryAll("[data-dashboard-target]").forEach((button) => {
    button.addEventListener("click", () =>
      showDashboardScreen(button.dataset.dashboardTarget),
    );
  });
...
function showDashboardScreen(screen) {
  ...
  queryAll("[data-dashboard-screen]").forEach((item) => {
    item.classList.toggle(
      "is-hidden",
      item.dataset.dashboardScreen !== state.currentScreen,
    );
  });
  queryAll("[data-dashboard-target]").forEach((button) => {
    button.classList.toggle(
      "active",
      button.dataset.dashboardTarget === state.currentScreen,
    );
  });
}
...
function syncRoleSwitch() {
  queryAll("#roleSwitch .role-pill").forEach((pill) =>
    pill.classList.toggle("active", pill.dataset.role === state.currentRole),
  );
}
...
function bindProfileMenu() {
  el("profileMenuToggle")?.addEventListener("click", () =>
    el("profileMenuPanel")?.classList.toggle("hidden"),
  );
  document.addEventListener("click", (event) => {
    if (!hasElement("profileMenu") || !hasElement("profileMenuPanel")) return;
    if (!el("profileMenu").contains(event.target))
      el("profileMenuPanel").classList.add("hidden");
  });
}
...
function updateAuthControls() {
  ...
  if (loggedIn && hasElement("profileMenuToggle")) {
    el("profileMenuToggle").textContent = (state.currentUser?.full_name || "П")
      .trim()
      .charAt(0)
      .toUpperCase();
  }
  ...
  const payload = {
    ...
    study_group:
      state.currentRole === "STUDENT"
        ? el("registerGroup").value || null
        : null,
    course:
      state.currentRole === "STUDENT" && el("registerCourse").value
        ? Number(el("registerCourse").value)
        : null,
    department:
      state.currentRole === "TEACHER" ? getSelectedDepartment() : null,
    program_code:
      state.currentRole === "STUDENT" ? getSelectedProgramCode() : null,
    subject_ids:
      state.currentRole === "TEACHER" ? getSelectedTeacherSubjectIds() : [],
  };
...
async function loadSubjects() {
  ...
  if (hasElement("subjectSelect")) {
    el("subjectSelect").innerHTML = state.subjects
      .map(
  ...
  if (hasElement("teacherSubjectsChecklist")) {
    ...
    `,
      ...
      .join("");
    queryAll("[data-teacher-subject]").forEach((item) =>
      item.addEventListener("change", renderSelectedTeacherSubjects),
    );
  }
}

async function loadUniversityCatalog() {
  if (!hasElement("registerFaculty") && !hasElement("ratingFacultyFilter"))
    return;
  state.universityCatalog = await api("/api/university/catalog", {
    headers: {},
  });
  ...
  if (hasElement("registerFaculty")) {
    el("registerFaculty").innerHTML = state.universityCatalog
      ...
      .join("");
    el("registerFaculty").addEventListener(
      "change",
...
function syncInstituteDependentFields() {
  ...
  const institute = state.universityCatalog.find(
    (item) => item.short_name === instituteCode,
  );
...
function getSelectedProgramCode() {
  return (
    document.querySelector('input[name="programCode"]:checked')?.value || null
  );
}

function getSelectedDepartment() {
  return (
    document.querySelector('input[name="departmentCode"]:checked')?.value ||
    null
  );
}

function getSelectedTeacherSubjectIds() {
  return queryAll("[data-teacher-subject]:checked").map((input) =>
    Number(input.value),
  );
}
...
function renderSelectedTeacherSubjects() {
  ...
  const selected = queryAll("[data-teacher-subject]:checked").map((input) =>
    input.parentElement.textContent.trim(),
  );
  el("teacherSubjectsSelected").textContent = selected.length
...
function renderProfile() {
  ...
  const teacherSubjects = user.teaching_subjects?.length
    ? user.teaching_subjects.map((s) => s.name).join(", ")
    : "—";
  const profileRows = [
...
function renderRoleCapabilities() {
  ...
  if (!state.currentUser) {
    el("roleCapabilities").innerHTML =
      '<div class="empty-state">После входа отобразятся доступные функциональные блоки.</div>';
    return;
  }
...
function toggleRoleWidgets() {
  ...
  queryAll(".student-only").forEach((item) =>
    item.classList.toggle("hidden-by-role", role !== "STUDENT"),
  );
  queryAll(".teacher-only").forEach((item) =>
    item.classList.toggle("hidden-by-role", role !== "TEACHER"),
  );
  const studentScreens = new Set(["builder", "runner", "stats"]);
  const teacherScreens = new Set(["moderation", "teacherStats"]);
  if (
    (role !== "STUDENT" && studentScreens.has(state.currentScreen)) ||
    (role !== "TEACHER" && teacherScreens.has(state.currentScreen))
  ) {
...
async function loadPublicData() {
  const [tests, ratings] = await Promise.all([
    api("/api/tests", { headers: {} }),
    api("/api/ratings", { headers: {} }),
  ]);
...
function renderStudentRatings() {
  ...
  el("studentRatingsList").innerHTML = top.length
    ? top
        .map(
          (item) =>
            `<div class="list-item"><strong>#${item.position} ${item.student_name}</strong><p>${item.subject_name}: ${item.total_score}</p></div>`,
        )
        .join("")
...
function renderTeacherStatsFilters() {
  ...
  el("teacherStatsSubject").innerHTML = names.length
...
function renderTeacherStats() {
  ...
  const items = state.ratings
    .filter((item) => item.subject_name === subjectName)
    .sort((a, b) => b.total_score - a.total_score);
  if (!items.length) {
    el("teacherStatsBox").innerHTML =
      '<div class="empty-state">Нет статистики по выбранному предмету.</div>';
    return;
  }
  ...
  el("teacherStatsBox").innerHTML = `
    ...
    ${items
      ...
      .map(
        (item) =>
          `<div class="list-item"><strong>${item.student_name}</strong><p>${item.total_score} баллов · ${item.faculty || "—"}</p></div>`,
      )
      .join("")}
  `;
}
...
async function loadMyStats() {
  if (!state.token || state.currentUser?.role !== "STUDENT")
    return renderStats();
...
function renderStats(stats) {
  ...
  if (!stats) {
    el("statsBox").innerHTML =
      "После прохождения тестов здесь появится статистика.";
    return;
  }
  const breakdown =
    Object.entries(stats.subject_breakdown)
      .map(([subject, score]) => `<li>${subject}: ${score}</li>`)
      .join("") || "<li>Пока нет рейтинга</li>";
  const attempts =
    stats.latest_attempts
      .map(
        (attempt) =>
          `<li>${attempt.subject_name}: ${attempt.score}/${attempt.max_score}</li>`,
      )
      .join("") || "<li>Нет попыток</li>";
  el("statsBox").innerHTML = `
...
async function loadAchievements() {
  ...
  if (!state.token || state.currentUser?.role !== "STUDENT")
    return (el("achievementsList").innerHTML = "");
  const items = await api("/api/achievements/me");
  el("achievementsList").innerHTML = items.length
    ? items
        .map(
          (item) =>
            `<div class="list-item"><strong>${item.name}</strong><p>${item.description}</p></div>`,
        )
        .join("")
...
function renderTests(tests) {
  ...
  if (!tests.length)
    return (el("testsList").innerHTML =
      '<div class="empty-state">Опубликованных тестов пока нет.</div>');
  ...
  queryAll("[data-open-test]").forEach((button) =>
    button.addEventListener("click", () => openTest(button.dataset.openTest)),
  );
}
...
async function openTest(testId) {
  ...
  el("testRunner").innerHTML =
    `<h3>${test.title}</h3><form id="attemptForm">${questionsHtml}<button class="primary-button" type="submit">Завершить тест</button></form><div id="attemptResult" class="message"></div>`;
...
async function submitAttempt(event) {
  ...
  const answers = state.selectedTest.questions.map((question) => {
    ...
    if (question.question_type === "TEXT_ANSWER") {
      answer.text_answer =
        event.target.querySelector(`[name="q_text_${question.id}"]`)?.value ||
        "";
    } else if (question.question_type === "MATCHING") {
      answer.matching_answer = Object.fromEntries(
        [
          ...event.target.querySelectorAll(`[name="q_match_${question.id}"]`),
        ].map((input) => [input.dataset.left, input.value]),
      );
    } else {
      answer.selected_option_ids = [
        ...event.target.querySelectorAll(`[name="q_${question.id}"]:checked`),
  ...
  try {
    ...
    el("attemptResult").textContent =
      `Результат: ${result.score}/${result.max_score} (${result.percentage}%).`;
...
function addQuestionBlock() {
  ...
  question
    ...
    .addEventListener("click", () => removeQuestionBlock(question));
  question
    .querySelector("[data-question-type]")
    .addEventListener("change", () => renderAnswerEditor(question));
...
function renderAnswerEditor(questionElement) {
  ...
  if (type === "TEXT_ANSWER") {
    block.innerHTML =
      '<div class="field-group"><label>Правильный ответ</label><input class="text-answer-input" type="text" required /></div>';
    return;
  }
  if (type === "MATCHING") {
    block.innerHTML =
      '<div class="section-title compact-title"><h3>Пары соответствия</h3><button class="soft-button compact" type="button" data-add-pair>Добавить пару</button></div><div class="matching-pairs stack"></div>';
    block
      .querySelector("[data-add-pair]")
      .addEventListener("click", () => addMatchingPair(questionElement));
    addMatchingPair(questionElement);
    addMatchingPair(questionElement);
    return;
  }
  block.innerHTML =
    '<div class="section-title compact-title"><h3>Ответы</h3><button class="soft-button compact" type="button" data-add-option>Добавить вариант</button></div><div class="answer-options stack"></div>';
  block
    .querySelector("[data-add-option]")
    .addEventListener("click", () => addAnswerOption(questionElement));
...
function removeQuestionBlock(questionElement) {
  ...
  if (
    !queryAll(".builder-question").length &&
    hasElement("constructorQuestions")
  ) {
...
function addAnswerOption(questionElement) {
  ...
  row.innerHTML =
    '<label class="answer-correct-toggle"></label><input class="answer-text-input" type="text" placeholder="Вариант ответа ' +
    optionIndex +
    '" required/><button class="soft-button compact" type="button" data-remove-option>Удалить</button>';
...
function syncAnswerInputTypes(questionElement) {
  ...
  questionElement.querySelectorAll(".answer-option").forEach((row, index) => {
    row.querySelector(".answer-correct-toggle").innerHTML =
      `<input type="${type === "SINGLE_CHOICE" ? "radio" : "checkbox"}" name="correct_${questionIndex}" ${index === 0 && type === "SINGLE_CHOICE" ? "checked" : ""}/><span>${type === "SINGLE_CHOICE" ? "Правильный" : "Корректный"}</span>`;
  });
}

function updateConstructorSummary() {
  if (hasElement("constructorSummary"))
    el("constructorSummary").textContent =
      `Вопросов: ${queryAll(".builder-question").length}`;
}
  ...
  const questions = queryAll(".builder-question").map((questionElement) => {
    ...
    const question = {
      text: questionElement
        .querySelector('[name="question_text"]')
        .value.trim(),
    ...
    if (type === "TEXT_ANSWER") {
      question.correct_answer = questionElement
        .querySelector(".text-answer-input")
        .value.trim();
    } else if (type === "MATCHING") {
      question.matching_pairs = [
        ...questionElement.querySelectorAll(".matching-pair"),
      ].map((row) => ({
      ...
      question.options = [
        ...questionElement.querySelectorAll(".answer-option"),
      ].map((row) => ({
...
async function submitLatestTest() {
  if (!state.myTests?.length)
    return (el("createMessage").textContent = "Сначала создайте тест.");
  const draft = state.myTests.find((item) => item.status === "DRAFT");
  if (!draft)
    return (el("createMessage").textContent =
      "У вас нет черновиков для отправки.");
  const test = await api(`/api/tests/${draft.id}/submit`, { method: "POST" });
  el("createMessage").textContent =
    `Тест "${test.title}" отправлен на модерацию.`;
  await loadMyTests();
}
...
function renderMyTests() {
  ...
  if (!state.myTests.length)
    return (el("myTestsList").innerHTML = "Созданные тесты появятся здесь.");
  el("myTestsList").innerHTML = state.myTests
    .map(
      (item) =>
        `<article class="list-item"><strong>${item.title}</strong><p>${item.subject_name}</p><p>Статус: ${item.status}</p></article>`,
    )
    .join("");
}
...
async function loadPendingTests() {
  ...
  if (!state.token || state.currentUser?.role !== "TEACHER")
    return (el("pendingList").innerHTML =
      "Только преподаватель может модерировать тесты.");
  ...
  queryAll("[data-approve]").forEach((button) =>
    button.addEventListener("click", () =>
      moderate(button.dataset.approve, "approve"),
    ),
  );
  queryAll("[data-reject]").forEach((button) =>
    button.addEventListener("click", () =>
      moderate(button.dataset.reject, "reject"),
    ),
  );
}
...
async function moderate(testId, action) {
  await api(`/api/tests/${testId}/moderate`, {
    ...
    body: JSON.stringify({
      action,
      comment: action === "approve" ? "Публикуем." : "Нужно доработать.",
    }),
  });
...
async function linkTelegram() {
  ...
  showToast(
    "Telegram link code",
    `${data.code} · действует ${data.ttl_seconds} сек.`,
    Math.max(1, data.ttl_seconds),
  );
  state.telegramLinkTimer = window.setTimeout(
    linkTelegram,
    Math.max(1, data.ttl_seconds) * 1000,
  );
  await loadProfile();
}
...
function showAchievementToasts(achievements = []) {
  achievements.forEach((achievement) =>
    showToast("Новое достижение", achievement, 10),
  );
}
